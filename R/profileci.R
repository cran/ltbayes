...
function(fmodel, y, zmin = -5, zmax = 5, lower = TRUE, upper = TRUE, level = 0.95, ...) 
{
	...
	f <- function(z, ...) {
		fmodel(z, prior = function(z) return(1), ...)$post - tmp$lmax + qchisq(level, 1)/2
	}
	if (lower) {
		tmp[c("lower","f.lower")] <- uniroot(f, lower = zmin, upper = tmp$zeta, y = y, ...)[c("root","f.root")]
		tmp$f.lower <- tmp$f.lower + tmp$lmax - qchisq(level, 1)/2
	}
	else {
	...
	if (upper) {
	  ...
	  tmp$f.upper <- tmp$f.upper + tmp$lmax - qchisq(level, 1)/2
	}
	else {
